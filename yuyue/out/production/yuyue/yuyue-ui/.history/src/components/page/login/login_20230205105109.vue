...
<script lang="ts" setup>
	...
	// http://localhost:8000/jwt/loginout?said=1001&saname=admin
	const toOut = () => {
		axios.post('api/jwt/loginout?said='+ruleForm.logDto.saId+"&saname="+ruleForm.saName,qs.parse(ruleForm),{
            headers: { 'content-type': 'application/json' }
        })
        .then(function(res){
			console.log(res)
			if(res.data.code==200 && res.data.success && res.data){
				ElMessage({
					message: '退出成功',
					type: 'success',
				})
